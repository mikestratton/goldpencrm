...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-2xl font-semibold dark-gold-font dark:text-white">Notes</h1>

            <div class="flex items-center gap-4 flex-1 max-w-2xl mx-4">
{{--                <form class="small-form" action="{{ route('notes.index') }}" method="GET" class="flex-1 flex gap-2">--}}
{{--                    <input type="text"--}}
{{--                           name="search"--}}
{{--                           placeholder="Search notes..."--}}
{{--                           value="{{ request('search') }}"--}}
{{--                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:text-white">--}}
...
{{--                    @endif--}}
{{--                </form>--}}
                <form class="small-form flex gap-2" action="{{ route('notes.index') }}" method="GET">
                    <input type="text"
                           name="search"
                           placeholder="Search notes..."
                           value="{{ request('search') }}"
                           class="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:text-white">
                    ...
                    @if($notes)
                        <table id="customers">
                            ...
                            @foreach($notes as $note)
                                <tr>
                                    <td><a href="{{ route('notes.edit', $note) }}"
                                           class="gold-link text-white py-1 px-1">
                                            {{$note->id}}
                                        </a></td>

                                    <td class="hidden sm:table-cell">{{ $note->title }}</td>
                                    <td class="hidden lg:table-cell">{{ $note->body }}</td>

                                    <td class="text-center">{{ $note->type_of_contact }}
